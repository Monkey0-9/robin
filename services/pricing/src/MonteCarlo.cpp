...
public:
    ...
    double price_asian_call() {
        ...
        for (unsigned int t = 0; t < num_threads; ++t) {
            threads.emplace_back([this, t, paths_per_thread, drift, diffusion, &partial_sums]() {
                ...
                for (size_t i = 0; i < paths_per_thread; ++i) {
                    ...
                    local_sum += std::max(0.0, avg - params_.strike_price);
                }
                partial_sums[t] = local_sum;
            });
        }
        ...
        auto end = std::chrono::high_resolution_clock::now();
        auto us = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        std::printf("[ASIAN] Parallel (%u threads) Arithmetic avg call = %.4f (%zu paths, %zu steps) in %lld us\n",
               num_threads, price, params_.paths_count, steps_, (long long)us);
        return price;
    }
