...
# Capture the caller-supplied key before importing Robin Swarm modules. app.config
...
def _episode_to_batch_item(graph_id: str, item: SourceEpisode, index: int) -> BatchAddItem:
    return BatchAddItem(
        ...
        source_description="Robin Swarm deep Zep Cloud validation corpus",
        metadata={
            "source": "robin-swarm_zep_deep_validation",
            "phase": item.phase,
            "sequence": index,
        },
    )
...
def _add_and_wait(client: Any, graph_id: str, item: SourceEpisode, timeout: int) -> str:
    episode = client.graph.add(
        ...
        data=item.data,
        created_at=item.created_at,
        source_description="Robin Swarm temporal Zep Cloud validation update",
        metadata={"source": "robin-swarm_zep_deep_validation", "phase": item.phase},
    )
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    graph_id = args.graph_id or f"robin-swarm_zep_deep_{stamp}"
    ...
    try:
        builder.create_graph("Robin Swarm Zep Cloud Deep Validation", graph_id=graph_id)
        created = True
        ...
        try:
            ...
            result["empty_attribute_ontology_probe"] = "accepted_after_robin-swarm_normalization"
        except Exception as error:
        ...
        batch = client.batch.create(
            metadata={
                "robin-swarm_operation_id": operation_id,
                "graph_id": graph_id,
        ...
        if updater_stats["items_sent"] != 5 or updater_stats["pending_episode_count"] != 0:
            raise AssertionError(f"unexpected Robin Swarm updater stats: {updater_stats}")
        result["robin-swarm_updater"] = updater_stats
        print("[zep-deep] Robin Swarm updater processed 5 mock activities", flush=True)
        ...
        if len(entity_context.related_edges) != len(complete_node_edges):
            raise AssertionError(
                "Robin Swarm entity context omitted incoming or outgoing node edges"
            )
...
def _compact_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "robin-swarm_updater": result.get("robin-swarm_updater"),
        "final": {
            key: final.get(key)
            for key in (
                "node_count",
