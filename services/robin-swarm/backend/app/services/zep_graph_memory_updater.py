...
logger = get_logger('robin-swarm.zep_graph_memory_updater')
    ...
    def _build_episode_payloads(
        ...
        for activity in activities:
            ...
            if len(text) > self.MAX_EPISODE_CHARS:
                marker = "... [truncated by Robin Swarm]"
        ...
        for payload_activities, combined_text in self._build_episode_payloads(activities):
            ...
            try:
                episode = self.client.graph.add(
                    ...
                    source_description="Robin Swarm simulation activity batch",
                    metadata={
                        "source": "robin-swarm_simulation",
