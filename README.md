...
| **Go Orchestrator & Gateway** | ✅ Working | Go | Service registry, TCP health probes (100ms), hot-reload config, CORS middleware, JWT+RBAC auth, preflight handlers, token-bucket rate limiting (1000 req/s), structured JSON logging (`slog`), Prometheus | Order routing prototype |
